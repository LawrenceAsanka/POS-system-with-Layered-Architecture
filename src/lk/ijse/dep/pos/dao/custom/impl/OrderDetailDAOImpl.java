...
public class OrderDetailDAOImpl implements OrderDetailDAO {
  ...
  public OrderDetail find(OrderDetailPK key) throws Exception {
    ...
    ResultSet rst = CrudUtil
        .execute("SELECT * FROM `OrderDetail` WHERE orderId=? AND itemCode=?", key.getOrderId(), key.getItemCode());
    if (rst.next()) {
  ...
  public boolean update(OrderDetail orderDetail) throws Exception {
    ...
    return CrudUtil
        .execute("UPDATE OrderDetail SET qty=?, unitPrice=? WHERE orderId=? AND itemCode=?", orderDetail.getQty(),
            orderDetail.getUnitPrice(), orderDetail.getOrderDetailPK().getOrderId(),
